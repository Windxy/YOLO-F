...
if show_animation:
    os.makedirs(os.path.join(results_files_path, "images", "detections_one_by_one_error"))
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for idx, detection in enumerate(dr_data):
            ...
            if show_animation:
                ...
                # cv2.waitKey() # show for 20 ms
                # save image to results
                output_img_path = results_files_path + "/images/detections_one_by_one/" + class_name + "_detection" + str(idx) + ".jpg"
                if status!="MATCH!":
                    output_img_path = results_files_path + "/images/detections_one_by_one_error/" + class_name + "_detection" + str(idx) + ".jpg"
                    cv2.imwrite(output_img_path, img)
                cv2.imwrite(output_img_path, img)
                # save the image with all the objects drawn to it
                cv2.imwrite(img_cumulative_path, img_cumulative)
                print(idx,"all is ",len(dr_data))
        #print(tp)
