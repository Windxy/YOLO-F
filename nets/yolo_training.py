...
def box_ciou(b1, b2):
    ...
    if True:  # TODO 使用ACIoU
        Smin = torch.min(b1_area, b2_area)
        Smax = torch.max(b1_area, b2_area)
        Sr = torch.abs(1 - Smin / Smax)
        aciou = ciou - alpha * v + Sr * 0.1
        return aciou
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        ...
        #  losses.
        mask_tem = mask.bool()
        pred_boxes_for_ciou = pred_boxes_for_ciou[mask_tem]
        ciou = (1 - box_ciou(pred_boxes_for_ciou, t_box[mask.bool()])) * box_loss_scale[mask.bool()]
