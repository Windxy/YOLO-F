...
def fit_one_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    print('Epoch:'+ str(epoch+1) + '/' + str(Epoch) + ' Total Loss: %.4f || Val Loss: %.4f ' % (total_loss/(epoch_size+1),val_loss/(epoch_size_val+1)))

    # print('Saving state, iter:', str(epoch+1))
    if total_loss/(epoch_size+1)<temp:
        torch.save(model.state_dict(), 'logs/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth'%((epoch+1),total_loss/(epoch_size+1),val_loss/(epoch_size_val+1)))
        temp = total_loss/(epoch_size+1)
#----------------------------------------------------#
#   检测精度mAP和pr曲线计算参考视频
...
if __name__ == "__main__":
    ...
    pretrain = False
    if pretrain:
        model_path = "logs/Epoch9-Total_Loss19.9503-Val_Loss12.4815.pth"
        # 加快模型训练的效率
    ...
    if True:
        ...
        optimizer = optim.Adam(net.parameters(),lr,weight_decay=5e-5)
        if Cosine_lr:
            lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=8, eta_min=1e-5)
        else:
